    ...
    <div id="main">
			<div class="content page-order-checkout checkout">
								...
								<ul class="address-item-list clear js-address-item-list">
									<li class="js-choose-address" @click="chooseReceive(index)" :class="{'selected-address-item':receiveIndex == index}" v-for="(item,index) in receiveInfo">
										<div class="address-item">
											...
											<div class="detail-section"> {{item.province}} {{item.city}} {{item.county}}
                                                <br> {{item.add}} 
                                            </div>
										</div>
				...
				<div class="gray-box">
						...
						<div class="invoice-detail"> 发票抬头： 
							<div class="radio-box"> 
								<label> 
									...
									<span class="blue-radio" @click="checkedInvoice(true)" :class="{'blue-radio-on' : incoice.presonal}"><a></a></span>  个人
								</label> 
								<label> 
									<input type="radio" class="hide"> 
									<span class="blue-radio" @click="checkedInvoice(false)" :class="{'blue-radio-on' : !incoice.presonal}"><a></a></span>  单位
								</label> 
							</div> 
							<div class="module-form-row form-item fn-hide js-invoice-title" v-if="!incoice.presonal"> 
                  ...
                  <div v-show="!incoice.name" class="verify-error">必填</div>
								</div> 
							</div> 
						</div> 
						...
						<div class="cart-table">
							<div class="cart-group js-cart-group">
								<div class="cart-items" v-for="item,index in checkGoods">
									...
									<div class="price">¥ {{item.count*item.price}}.00</div>
								</div>
...
export default {
    ...
    methods:{
        ...
        checkedInvoice(boole){
          this.incoice.presonal = boole
        },
        //提交订单
        submitOrder(){
          if(!this.incoice.presonal && !this.incoice.name) return
          let receiveInfo = this.receiveInfo[this.receiveIndex]
          if(this.incoice.presonal){
